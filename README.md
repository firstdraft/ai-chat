...
Note: similar to re-sending user images and files, if you generate another API request using the same chat, old images generated by the model in the conversation history will not be sent by default. If you really want to re-send old images, then you must set `previous_response_id` to `nil`:
